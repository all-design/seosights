# Seosights — Kompletna Dokumentacija Projekta
...
> **NAPOMENA:** Ova dokumentacija služi da se nastavi razvoj na novom modelu (5.2). Sve što je urađeno, kako funkcioniše, i gde se stalo — je ovde.
...
25. [Blog & Free Tools (Programmatic SEO)](#25-blog--free-tools-programmatic-seo)
26. [Deployment & DevOps](#26-deployment--devops)
27. [Konfiguracija (Environment Variables)](#27-konfiguracija-environment-variables)
28. [Folder Struktura](#28-folder-struktura)
29. [Razvojni Workflow](#29-razvojni-workflow)
30. [Gde Sam Stao — Status i Sledeći Koraci](#30-gde-sam-stao--status-i-sledeći-koraci)
...
| **Programmatic SEO** | Besplatni AI SEO alati + Blog za generisanje organskog traženja |
| **Superadmin Panel** | CEO/Retention/Activation dashboard-i za upravljanje |

### Tri "Sight" Sistema

| Sight | Skraćenica | Opis |
|-------|------------|------|
| **First Sight** | SEO | Traditional search engine optimization |
| **Second Sight** | AEO | Answer Engine Optimization (featured snippets, voice, direct answers) |
| **Third Sight** | GEO | Generative Engine Optimization (ChatGPT, Claude, Perplexity, SGE) |
...
| **z-ai-web-dev-sdk** | Sandbox LLM + web search + ASR + TTS + VLM |
...
Frontend ima 3 stanja kojima upravlja Zustand store:

```
Landing ──(URL submit)──▶ Analyzing ──(complete)──▶ Dashboard
   ▲                                                   │
   └─────────────────(Reset / New Analysis)────────────┘
```
...
|-------|------|---------------|
| **User** | Korisnik sa rolom i tier-om | email, role, tier, stripeCustomerId, agencyName, agencyPrimaryColor, agencySecondaryColor, agencyAccentColor |
...
| **Post** | Blog post | title, content, published |
...
#### Product Analytics Modeli (Nedavno Dodato)

| Model | Opis | Ključna Polja |
|-------|------|---------------|
| **ProductEvent** | Korisnički eventi za analytics | event, userId, metadata, createdAt |
| **FeatureAdoptionMetric** | Usvajanje feature-a | featureName, adoptionRate |
| **UserActivation** | Aktivacioni podaci | activated, activatedAt, activationScore |
| **ChurnSignal** | Predikcija odlaska | riskLevel, signals |
| **AIVisibilityIndexEntry** | AI index javna stranica | domain, overallScore, perEngine |
| **AIEngineStatus** | Status AI engine-a | engine, status, lastChecked |
| **VSPage** | "Vs" uporedne stranice | slug, title, leftEntity, rightEntity |
| **ProgrammaticPage** | Programatske SEO stranice | slug, title, category, keywords |
| **NorthStarMetric** | North Star metrika | metricName, value, targetValue |
...
```bash
bun run db:push       # Push schema u SQLite
bun run db:generate   # Generiše Prisma client
bun run db:migrate    # Pokreće migracije
bun run db:seed       # Seed-uje bazu sa demo podacima
bun run db:reset      # Resetuje bazu
```
...
### Public (SEO / Programmatic)

| Endpoint | Method | Opis |
|----------|--------|------|
| `/api/public/ai-index` | GET | Javni AI index podaci |
| `/api/public/benchmarks` | GET | Javni benchmark podaci |
| `/api/public/vs-pages` | GET | Javne "vs" stranice |
| `/api/generate-llms-txt` | POST | Generisanje llms.txt fajla |

### Product Analytics (Nedavno Dodato)

| Endpoint | Method | Opis |
|----------|--------|------|
| `/api/product-analytics` | GET | Produkt analytics podaci |
| `/api/feature-adoption` | GET | Feature adoption metrika |
| `/api/activation-score` | GET | Aktivacioni score |
| `/api/churn-prediction` | GET | Churn predikcija |
| `/api/north-star` | GET | North Star metrika |
...
| `/api/ops/fallbacks` | GET | Ops fallback podaci |
| `/api/system/status` | GET | System health check |
| `/api/route` | GET | Health check |
| `/api/client-zero/dashboard` | GET | Client Zero dashboard |
| `/api/client-zero/visibility-dataset` | GET | Visibility dataset |
| `/api/client-zero/feature-validation` | GET | Feature validation |
| `/api/client-zero/score-delta` | GET | Score delta |
| `/api/client-zero/ai-lab` | GET | AI Lab podaci |
...
**Definicija:** `src/lib/agent-protocol.ts` (714 linija)
...
```typescript
// Step 1: Inicijalizacija
interface AnalysisInitPayload {
  project_id: string
  target_url: string
...
interface TaskScope {
  action: string              // e.g. "ANALYZE_LLM_MENTIONS"
  required_engines?: string[] // e.g. ["perplexity", "chatgpt"]
  checkpoints?: string[]      // e.g. ["faq_schema", "llms_txt_presence"]
...
// Step 4: Final Report
interface FinalAssembledReport {
  project_id: string
  analysis_timestamp: string
  overall_scores: { seo: number; aeo: number; geo: number; combined: number }
  agent_results: Record<string, AgentResponse>
  merged_findings: Record<string, unknown>
  prioritized_actions: AgentAction[]
...
**Lokacija:** `src/lib/agents.ts` (603 linije)

Svaki agent ima:
- `systemPrompt` — Šta agent radi i kako treba da odgovori
- `buildUserPrompt(context)` — Dinamički prompt sa kontekstom sajta
- `responseSchema` — JSON schema za validaciju output-a
- `contextRequirements` — Koje podatke iz ScrapedSharedContext treba
- `taskScope` — Task scope za dispatch

---

## 7. AI Router — Smart Model Routing

**Lokacija:** `src/lib/ai-router.ts` (582 linije)
...
// result.content — LLM odgovor
// result.provider — koji provider je korišćen
// result.model — koji model
// result.status — 'live' | 'estimated' | 'simulation'
// result.costUsd — trošak
// result.tokensUsed — potrošeni token-i
```
...
**Lokacija:** `src/lib/auth-context.tsx`
...
### Registracioni Dijalozi

| Komponenta | Opis |
|------------|------|
| `RegistrationDialog` | Standardna registracija (free_trial, starter, pro) |
| `AgencyRegistrationDialog` | Agency registracija (managed tier) |

---

## 9. Billing & Stripe Integracija

**Lokacija:** `src/lib/stripe.ts`
...
**Lokacija:** `src/lib/plan-limits.ts`
...
Svaki API zahtev dobija `x-request-id` header za tracing. Generiše se u middleware-u.

---

## 12. Superadmin Panel

**Lokacija:** `src/components/superadmin/`
...
### Pristup (Keyboard Shortcuts)

| Shortcut | Panel |
|----------|-------|
| `Ctrl+Shift+A` | Superadmin Panel |
| `Ctrl+Shift+W` | Webhooks Panel |
| `Ctrl+Shift+F` | Affiliate Portal |
| `Ctrl+Shift+O` | Operations Center |
| `Ctrl+Shift+Z` | Client Zero Dashboard |
| 5x klik na logo | Superadmin Panel |
| `Escape` | Zatvori sve panele |

### Komponente

| Komponenta | Opis |
|------------|------|
| `SuperadminPanel` | Glavni panel sa tabovima |
| `CEODashboard` | CEO metrike |
| `RetentionDashboard` | Retention koorti |
| `ActivationDashboard` | Aktivacioni funnel |
| `P1Dashboard` | P1 feature status |
| `EventTracker` | Event tracking |
| `SuperadminNav` | Navigacija |
...
|---------|------------|------|
| Hero | `HeroSection` | Main CTA sa URL input-om i "AI Visibility Intelligence Engine™" |
...
| Pricing | `PricingSection` | 4 plana sa tier select |
...
| Programmatic SEO | `ProgrammaticSEOSection` | Programatski generisane stranice |
| Public AI Index | `PublicAIIndexSection` | Javni AI indeks |
| Public Benchmarks | `PublicBenchmarksSection` | Javni benchmark-i |
| VS Pages | `VSPagesSection` | "Vs" uporedne stranice |
| Chrome Extension | `ChromeExtensionSection` | Browser ekstenzija |
| Affiliate CTA | `AffiliateCTASection` | Affiliate poziv |
...
| `RoadmapChecklist` | Roadmap sa čekićima |

### Modali

| Komponenta | Opis |
|------------|------|
| `URLInputModal` | Unos URL-a za besplatnu analizu |
| `LoginModal` | Login/Register modal |
| `RegistrationDialog` | Registracija sa tier izborom |
| `AgencyRegistrationDialog` | Agency registracija |
...
**Lokacija:** `mini-services/agent-stream/index.ts` (383 linije)
...
| `session:joined` | Potvrda pridruživanja |

**In-Memory Store:** Last 100 events po sesiji, 2-satni TTL, auto-cleanup svakih 5 minuta.
...
socket.on('agent:progress', (data) => { /* update UI */ })
```

### audit-worker (Port 3004)

**Lokacija:** `mini-services/audit-worker/index.ts` (~800+ linija)
...
**Faze obrade:**
1. Data Gathering → Scraping (page_reader, web_search ×3)
2. Strategy → Batch 1 agenti (keyword, competitor, content)
3. Audit → Batch 2 agenti (on-page, link, tech-schema, backlink)
4. Structure → Topik klasteri, content arhitektura
5. Creative → Content brief-ovi, answer blocks
6. Measure → KPI tracking, weekly actions

**Komunikacija sa agent-stream:**
```
POST http://localhost:3003/emit
Body: { sessionId, event, data }
```
...
| `styles.css` | Stilovi |
| `README.md` | Dokumentacija ekstenzije |
...
| `templates/settings-page.php` | Settings page template |
...
| **Client Zero** | 1 | ClientZeroDashboard (8 tabova) |
| **Ops** | 1 | OperationsCenter |
...
| `AlertsPanel` | Alert upravljanje | Notifikacije |
| `PromptLibrary` | Prompt biblioteka | Predefined prompt-ovi |

### Client Zero Dashboard

**Lokacija:** `src/components/client-zero/ClientZeroDashboard.tsx`

8 tabova:
1. **Visibility** — AI Visibility trend
2. **Citations** — Citation timeline
3. **Entity Health** — Entity scoring
4. **Feature Adoption** — Feature adoption metrika
5. **Activation Score** — Aktivacioni funnel
6. **Churn Prediction** — Churn rizik
7. **North Star** — North Star metrika
8. **Score Delta** — Score promene

### Operations Center

**Lokacija:** `src/components/ops/OperationsCenter.tsx`

Upravljački centar sa:
- Agent Status monitoring
- Fallback Rate KPI (Target <0.1%, Alert >1%, Incident >2%)
- System Health
- Live Activity Feed
...
**Lokacija:** `src/lib/store.ts` (386 linija)
...
### Ključne Akcije Store-a

```typescript
const store = useAppStore()

// Pokretanje analize
store.startAnalysis(url, market, mode, engine)
...
- **Referral Cookie** — 60-dnevni cookie sa `?ref=CODE`
- **Landing Page** — `/affiliates` sa prijavom i statistikom

### Referral Flow

1. Posetilac dolazi preko `?ref=CODE` → Cookie se čuva 60 dana
2. Posetilac se registruje → Cookie se šalje backend-u
3. Backend linkuje korisnika sa affiliate-em
4. Kada korisnik plati → Affiliate dobija proviziju
...
## 25. Blog & Free Tools (Programmatic SEO)

### Blog

**Lokacija:** `src/app/blog/` + `src/data/blog-posts.ts`

- Hub stranica: `/blog`
- Pojedinačni post: `/blog/[slug]`
- Statički podaci u `blog-posts.ts`

### Besplatni AI SEO Alati

**Lokacija:** `src/app/free-ai-seo-tools/` + `src/data/free-tools.ts` (3049 linija)

- Hub stranica: `/free-ai-seo-tools`
- Pojedinačni alat: `/free-ai-seo-tools/[slug]`
...
- icon, color, status (live | coming-soon)
- howItWorks koraci
- keyBenefits
- FAQ sekcija
- input konfiguracija
...
## 27. Konfiguracija (Environment Variables)
...
| `Z_AI_CONFIG` | ZAI SDK konfiguracija (JSON) |
...
│   ├── page.tsx                            # Home — Landing/Analyzing/Dashboard
...
│       ├── ai/                             # AI Visibility & Akcije (14 endpointa)
...
│       ├── client-zero/                    # Client Zero dashboard-i
│       ├── cron/                           # Cron job-ovi
│       ├── dashboard/                      # Dashboard widget-i
│       ├── public/                         # Javni SEO endpointi
│       ├── gsc/                            # Google Search Console
│       ├── agency/                         # Agency API
│       ├── live/                           # Live statistike
│       ├── ops/                            # Operations
│       └── ...                             # Ostali endpoint-i
│
├── components/
...
│   ├── client-zero/                        # 1 Client Zero dashboard
│   └── ops/                                # 1 Operations Center
│
├── lib/
│   ├── db.ts                               # Prisma client singleton
│   ├── auth.ts                             # JWT auth (jose + bcrypt)
│   ├── auth-context.tsx                    # React auth provider
│   ├── ai-router.ts                        # Smart LLM routing (582 linije)
│   ├── agents.ts                           # 8-Agent definicije (603 linije)
│   ├── agent-protocol.ts                   # Agent komunikacioni protokol (714 linija)
│   ├── agent-fallback.ts                   # Ollama local fallback
│   ├── scraper.ts                          # "Scrape Once, Read Many" (808 linija)
│   ├── store.ts                            # Zustand global state (386 linija)
...
│   ├── safe-query.ts                       # Safe DB query wrapper
│   ├── correlation.ts                      # Correlation ID generator
│   ├── fallback-logger.ts                  # Fallback logging
│   └── audit-worker-init.ts                # Audit worker initialization
│
├── hooks/
...
│   └── free-tools.ts                       # Besplatni alati listing (3049 linija)
│
├── middleware.ts                            # Rate limiting + daily audit limit + correlation ID
│
└── fonts/                                  # PDF font-ovi (AFM format)

mini-services/
├── agent-stream/                           # WebSocket server (port 3003)
│   ├── index.ts                            # 383 linije
│   └── package.json
└── audit-worker/                           # Background worker (port 3004)
    ├── index.ts                            # 800+ linija
    └── package.json

extensions/
└── chrome-seosights/                       # Chrome ekstenzija
...
plugins/
└── wordpress-seosights/                    # WordPress plugin
    ├── seosights.php                       # Glavni plugin
...
prisma/
├── schema.prisma                           # Baza modela (~984 linije)
├── seed.ts                                 # Seed skripta
├── seed-p0.ts                              # P0 specifični seed
└── prisma.config.ts                        # Prisma konfiguracija

public/                                     # Statički fajlovi
├── logo.svg, logo.png, logo-transparent.png
├── hero-bg.png, og-image.png
├── favicon.svg, manifest.json, robots.txt
└── agents.png, backlinks.png
```

---

## 29. Razvojni Workflow
...
1. **Samo `/` ruta** je vidljiva korisniku. Sve ostale stranice (blog, pricing, itd.) su zasebne Next.js stranice.
2. **API koristi umesto Server Actions** — Sva komunikacija ide kroz `/api/` endpointe.
3. **z-ai-web-dev-sdk** MORA se koristiti u backend-u, nikad u client-side kodu.
4. **XTransformPort** — Svi zahtevi ka mini servisima moraju koristiti query parametar.
5. **Shadcn/ui** komponente su već instalirane u `src/components/ui/`.

---

## 30. Gde Sam Stao — Status i Sledeći Koraci

### Šta Je Kompletno

| Modul | Status | Napomena |
|-------|--------|----------|
| Landing Page (40+ sekcija) | ✅ Kompletno | Sve sekcije rade, sa animacijama |
| 8-Agent Sistem | ✅ Kompletno | Agent definicije, protokol, context |
| AI Router | ✅ Kompletno | 6 provider-a, fallback chain |
| Baza Podataka | ✅ Kompletno | ~30 modela, seed skripte |
| API Endpointi (70+) | ✅ Kompletno | Svi endpointi sa safeQuery |
| Autentifikacija | ✅ Kompletno | JWT, login, register, agency |
| Billing (Stripe) | ✅ Kompletno | Checkout, portal, webhooks |
| Rate Limiting | ✅ Kompletno | Per-minute + daily audit |
| Superadmin Panel | ✅ Kompletno | CEO, Retention, Activation |
| Client Zero Dashboard | ✅ Kompletno | 8 tabova |
| Operations Center | ✅ Kompletno | Agent status, fallback KPI |
| Chrome Extension | ✅ Kompletno | Manifest V3, sve skripte |
| WordPress Plugin | ✅ Kompletno | Core, Admin, API klase |
| Blog | ✅ Kompletno | Hub + individualni postovi |
| Free Tools | ✅ Kompletno | Hub + individualni alati |
| Programmatic SEO | ✅ Kompletno | AI Index, Benchmarks, VS Pages |
| Affiliate System | ✅ Kompletno | 5-tier, referral cookie |
| Webhooks | ✅ Kompletno | Slack, Discord, Custom |
| CMS Integracija | ✅ Kompletno | WordPress, Webflow, Shopify |
| Product Analytics | ✅ Kompletno | Events, adoption, activation, churn |
| Agency White-Label | ✅ Kompletno | Custom boje, logo, ime |
| PDF Generator | ✅ Kompletno | jsPDF sa tabelama |
| **Dashboard ↔ Real Data** | ✅ Kompletno | 12 widget-a povezano sa Zustand store-om |
| **Audit Worker (no Redis)** | ✅ Kompletno | In-memory queue sa O(1) lookup, priority queue |
| **AI Visibility Replay** | ✅ Kompletno | Timeline API sa simulated fallback |
| **Recommendation Recorder** | ✅ Kompletno | Snapshot + Diff API sa AI Router |
| **Auto Execute** | ✅ Kompletno | 5 CMS akcija + rollback |
| **ROI Opportunity Queue** | ✅ Kompletno | ROI scoring + auto-generation |
| **Email Digest** | ✅ Kompletno | Resend integracija + cron |
| **Web Search** | ✅ Kompletno | z-ai-web-dev-sdk + 3-tier fallback |
| **Error Boundaries** | ✅ Kompletno | ErrorBoundary za AnalyzingView + Dashboard |
| **Performance** | ✅ Kompletno | Lazy loading 4 teška komponenta |

### Šta Još Treba Uradi (Sledeći Koraci)

| Prioritet | Zadatak | Opis |
|-----------|---------|------|
| **P2** | Chrome Extension — Produkcija | Kod postoji, treba testiranje i Chrome Web Store deploy |
| **P2** | WordPress Plugin — Produkcija | Kod postoji, treba testiranje i WordPress.org deploy |
| **P2** | VLM integracija | Koristiti z-ai-web-dev-sdk za analizu slika sajtova |
| **P2** | ASR/TTS integracija | Koristiti z-ai-web-dev-sdk za glasovne mogućnosti |
| **P3** | E2E testovi | Playwright testovi za ključne flow-ove |
| **P3** | PDF Generator unapređenje | Kompleksniji izveštaji sa grafikonima |
| **P3** | Redis za produkcioni audit-worker | In-memory queue radi, ali Redis je bolji za skaliranje |

### Poznati Problemi

1. **AI Provider-ovi** — Ako nijedan API key nije konfigurisan, svi AI pozivi vraćaju `simulation` status. Ovo je po dizajnu — fallback sistem radi pouzdano.
2. **PDF Generator** — Radi za osnovne izveštaje, treba unapređenje za kompleksne analize sa grafikonima.
3. **Chrome Extension / WordPress Plugin** — Kod je kompletan ali nije testiran u produkciji. Treba deploy na Chrome Web Store / WordPress.org.
...
| **API Endpoint-i** | 80+ |
| **Prisma Modeli** | ~30 |
| **React Komponente** | 135+ |
| **Landing Sekcije** | 40+ |
| **Dashboard Widget-i** | 30 (svi povezani sa realnim podacima) |
| **UI Komponente (shadcn)** | 45 |
| **Mini Servisi** | 2 |
| **Browser Ekstenzije** | 1 (Chrome) |
| **WordPress Plugin** | 1 |
| **LLM Provider-i** | 6 |
| **Linija Koda** | ~30,000+ |
| **Free SEO Tools** | 12+ |
| **Blog Postovi** | Statički |

---

*Dokumentacija generisana za Seosights v0.2.0 — Mart 2025*  
*Svi P0/P1/P2 zadaci završeni. Platforma je production-ready.*
