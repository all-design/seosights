...
export async function GET(request: NextRequest) {
  try {
    ...
    // needsRewrite filter not available in current schema
        ...
        articles: [
          {
            ...
            publishedAt: null,
            reviewResults: null,
            metadata: '{"faqEntries":[{"q":"What is AI Visibility for Dentists?","a":"AI Visibility for Dentists is the process of optimizing your dental practice content so AI engines cite you."}],"externalCitations":[{"title":"Wikipedia - Search Engine Optimization","url":"https://en.wikipedia.org/wiki/Search_engine_optimization"}]}',
            status: 'review',
            createdAt: new Date(Date.now() - 86400000).toISOString(),
            updatedAt: new Date(Date.now() - 86400000).toISOString(),
            brief: { keywordTarget: 'AI Visibility for Dentists', pillar: 'geo', topic: 'AI Visibility for Dentists' },
          },
          {
            id: 'seed-article-2',
            ...
            reviewResults: null,
            metadata: '{"faqEntries":[{"q":"Is SeoSights better than Surfer SEO?","a":"SeoSights specializes in AI visibility optimization while Surfer SEO focuses on traditional content optimization."}],"externalCitations":[]}',
            status: 'published',
            createdAt: new Date(Date.now() - 259200000).toISOString(),
            updatedAt: new Date(Date.now() - 172800000).toISOString(),
            brief: { keywordTarget: 'SeoSights vs Surfer SEO', pillar: 'geo', topic: 'SeoSights vs Surfer SEO' },
          },
        ],
        total: 2,
        page,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingArticles = brief.articles || []
    const nextVersion = existingArticles.length > 0 ? 2 : 1

    // Parse brief data
    const outline: Array<{ h2: string; h3s: string[]; points: string[] }> = JSON.parse(brief.briefContent || '[]')
    const secondaryKeywords: string[] = JSON.parse(brief.opportunityData || '[]')
    const entityTargets: Array<{ entity: string; type: string; reason: string }> = []

    // Generate article content
    const { title, slug, content, wordCount } = generateArticleContent(
      brief.keywordTarget || brief.topic,
    ...
    const article = await db.contentArticle.create({
      data: {
        ...
        content,
        wordCount,
        format: brief.pillar || 'blog',
        pillar: brief.pillar || 'geo',
        seoScore: 0,
        aeoScore: 0,
        geoScore: 0,
        ...
        internalLinks,
        status: 'draft',
        metadata: JSON.stringify({ faqEntries, externalCitations, metaTitle, metaDescription, ogImageUrl: null }),
      },
    })
