    ...
    for (const reviewer of REVIEWERS) {
      ...
      const result = await db.qAReviewerResult.create({
        data: {
          ...
          score,
          issues: issueCount,
          details: JSON.stringify(details[reviewer] || []),
          summary: REVIEWER_SUMMARIES[reviewer],
          recommendations: JSON.stringify(recommendations[reviewer] || []),
        },
      })
    ...
    for (let i = 6; i >= 1; i--) {
      ...
      const historicalRun = await db.qARun.create({
        data: {
          ...
          duration: randomInt(120000, 180000),
          productScore: base,
          uxScore: base - randomInt(2, 5),
          engineeringScore: base - randomInt(0, 3),
          researchScore: base - randomInt(4, 8),
          conversionScore: base - randomInt(8, 14),
          enterpriseScore: base - randomInt(5, 10),
          ...
          pagesTested: randomInt(15, 22),
          clicksTested: randomInt(280, 400),
          apisTested: randomInt(35, 55),
          formsTested: randomInt(12, 22),
