...
// ── Mock Data ─────────────────────────────────────────────────
const LEADERBOARD: CompetitorRow[] = [
  { id: '1', domain: 'rushforsheets.com', score: 78, previousScore: 75, isYou: false },
  { id: '2', domain: 'yourdomain.com', score: 71, previousScore: 68, isYou: true },
  { id: '3', domain: 'sheetmagic.io', score: 65, previousScore: 67, isYou: false },
  { id: '4', domain: 'formulabot.com', score: 52, previousScore: 52, isYou: false },
  { id: '5', domain: 'excelforum.com', score: 41, previousScore: 44, isYou: false },
]
...
export default function AICompetitorRace({ onStartFree }: AICompetitorRaceProps) {
  ...
  const yourData = LEADERBOARD.find(r => r.isYou)!
  const yourRank = LEADERBOARD.findIndex(r => r.isYou) + 1
  const leaderScore = LEADERBOARD[0].score
  const gapToLeader = leaderScore - yourData.score
        ...
        >
          <Card className="border border-white/10 bg-white/[0.03] backdrop-blur-xl">
            <CardContent className="p-4 sm:p-6">
              ...
              {/* Overtake Alert */}
              {yourData.score > yourData.previousScore && (
                <motion.div
                ...
                >
                  ...
                  <span className="text-xs text-emerald-300 font-medium">
                    Overtake Alert! You gained +{yourData.score - yourData.previousScore} points and are closing in on #{yourRank - 1 > 0 ? yourRank - 1 : 1}
                  </span>
                </motion.div>
              )}
            </CardContent>
                    ...
                    >
                      <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <div className="shrink-0 flex flex-col items-end gap-0.5">
                          ...
                          {getChangeIndicator(row.score, row.previousScore)}
                        </div>
                      </div>
                    </motion.div>
                  )
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50 mt-0.5">
                    You&apos;re only <span className="text-amber-400 font-bold">{gapToLeader} points</span> behind <span className="text-white/70 font-medium">{LEADERBOARD[0].domain}</span>. Fix 2 quick wins to take the lead.
                  </p>
                </div>
              </div>
              <Button
